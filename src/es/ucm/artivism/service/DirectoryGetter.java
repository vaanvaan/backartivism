...
public class DirectoryGetter {
	String baseImgDir = null;
	String basePostDir = null;
	String locationApiKey = null;
	String baseDir = null;
	
	private void loadLocalParameters() throws IOException {
		...
		try {
			...
			baseImgDir = prop.getProperty("base_img_directory");
			basePostDir = prop.getProperty("base_post_directory");
			locationApiKey = prop.getProperty("location_api_key");
	...
	public List<PostVO> getPosts(final Integer maxPosts, final String baseUrl) {
		...
		if(baseDir == null || baseImgDir == null || basePostDir == null || locationApiKey == null){
			try {
				loadLocalParameters();
			} catch (IOException e) {
